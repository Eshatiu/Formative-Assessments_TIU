...
#### Remove all the unnecessary punctuation symbols denoted in the string below, convert to white space
unnecessary_punctuation = r"&$@[].,'#()-\"!?’_;:/"

# #string
mydict = str(jsondata.values())
...
# # for k, v in mydict.items():
#     mydict.update({k.upper(), v.upper()})

# # #removal
html_list = [ "<P>", "</P>", "<B>", "</B>", "<I>", "</I>"]
...
for v in mydict.items():
    if i == unnecessary_punctuation[i]
        v.replace(html_ist[i]), " "
...
for k, v in wordcount_dictionary.items()
    if k >= 3:
        wordcount_dictionary.append(k)
        wordcount_dictionary.append(len(k))
...
for i in optimal_F
    1000*int(F) + 500*int(F) + 50*int(F)
        F += 1
for i in optimal_R
    600*int(R) + 750*int(R) + 45*int(R) 
        R += 1
for i in optimal_C
    750*int(C) + 300*int(C) + 75*int(C) 
        C += 1
...
def Collatz(start_number):
    ...
    collatz_dict ["Collatz Sequence"] = list_of_numbers
    collatz_dict ["Collatz Length"] = len(list_of_numbers)
    collatz_dict ["Max Collatz"] = max(list_of_numbers)
    collatz_dict ["Sequence Sum"] = sum(list_of_numbers)
...
def Collatz_winner(number_list):
    ...
    for i in range (x, y):
        list(map(collatz_dict["Collatz Length"], i))
        length_list.append[i]

    for i in range (x, y):
        list(map(collatz_dict["Max Collatz"], i))
        size_list.append[i]

    
    for i in range (x, y)
        if i == longest and i == largest:
            return [i]
