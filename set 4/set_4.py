...
def bin_to_dec(binary_string):
    ...
    reversed_list = []
    new_string = int(binary_string)

    while new_string > 0:
        reversed_list.append(new_string % 10)
        new_string = (new_string - new_string % 10) // 10
    
    sum = 0
    length_list = len(reversed_list)
    
    for i in range(length_list):
        sum += reversed_list[i] * (2**i)
...
def telephone_cipher(message):
    ...
    dict_list = list(encoder_dict.values())
    encrypt_list = []
    
    length_dict = len(dict_list)

    encrypted_message = ""
    dict_number = 0

#match to list to find corresponding dictionary value
#use alphabet to find index
    for element in message_list: 
        if element == " ":
            encrypted_message += "0"
      
        else:   
            element_position = ((ord(element) - ord("A")) + 1) % 26
            dict_number = dict_list[element_position]
            encrypt_list.append(dict_number)

    length_encrypt = len(encrypt_list)

#add "_" to values beginning with the same number
    for i in range(length_encrypt):
        if encrypt_list[i][0] == encrypt_list[(i+1) % length_encrypt][0]:
            encrypt_list[i] = encrypt_list[i] + "_"

    adjusted_string = str("".join(encrypt_list))
    adjusted_list = list(adjusted_string)

#remove "_" if number is the last in the list
    if adjusted_list[-1] == "_":
        adjusted_list.pop(-1)

    final_string = str("".join(adjusted_list))
...
# turn into list
    number_list = list(telephone_string)
    res = []
    curr_group = []
    
# group by element in nested list
# combine elements that are more than 2
    for i, x in enumerate(number_list):
        if i == 0 or x != number_list[i-1]:
            curr_group = [x]
...
# flatten list
    ...
    key_list = list(decipher_dict.keys())
    value_list = list(decipher_dict.values())
    message = ""
    ...
    for i in range(len(res)):
        if res[i] == "_":
            ...
            position = key_list.index(res[i])
            letter = value_list[position]
            message += letter
    return message
